...
from pintext import UnitGenerator, get_unit_registry
from pintext.converters import ensure_units, to_quantity
from pintext.exceptions import DimensionalityError
...
class TestEnsureUnits:
    ...
    def test_str(self):
        # Unit-carrying strings are parsed, not used as magnitudes
        assert ensure_units("2 m", default_units=ureg.km) == 2 * ureg.m
        assert ensure_units(
            "2 m", default_units=ureg.km, convert=True
        ) == ureg.Quantity(0.002, "km")
        # Strings without units are treated as unitless values
        assert ensure_units("2", default_units=ureg.km) == 2 * ureg.km
        # Unparsable strings raise
        with pytest.raises(pint.UndefinedUnitError):
            ensure_units("text", default_units=ureg.km)
...
class TestToQuantity:
    ...
    @pytest.mark.parametrize(
        "value",
        [
            "text",  # UndefinedUnitError
            "(",  # tokenize.TokenError
            "3 +",  # AssertionError
            "1/0",  # ZeroDivisionError
        ],
        ids=["undefined_unit", "token_error", "assertion_error", "arithmetic_error"],
    )
    def test_unparsable_str(self, value):
        """
        Pint's parse failures are passed through (default) or raise (strict mode).
        """
        assert to_quantity(value) is value

        with pytest.raises(ValueError, match="Conversion of value to quantity failed"):
            to_quantity(value, strict=True)
