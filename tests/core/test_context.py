...
def test_unit_context_init():
    """
    Unit tests for :meth:`pintext._context.UnitContext()`.
    """
...
def test_unit_context_non_idempotent_key_converter():
    """The key converter is applied exactly once per key."""
    unit_context = UnitContext({"length": ureg.m}, key_converter=lambda k: f"u_{k}")
    assert unit_context.registry == {"u_length": UnitGenerator(ureg.m)}
    assert unit_context.get("length") == ureg.m

    unit_context.register("time", ureg.s)
    assert unit_context.registry["u_time"] == UnitGenerator(ureg.s)
...
def test_unit_context_override():
    ...
    with pytest.raises(TypeError), unit_context.override(1.0):
        pass
