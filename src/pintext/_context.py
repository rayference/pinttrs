...
class UnitContext:
    ...
    :param registry:
        Unit generator registry. Keys can be any hashable type, but :class:`str`
        or :class:`~enum.Enum` are recommended. Defaults to an empty dictionary.
    ...
    :param interpret_str:
        ...
        unit generators as :class:`str`. Defaults to ``False``.

    :param ureg:
        Unit registry used for string-to-units interpretation. If ``None``, the
        default registry is used (see :func:`.get_unit_registry`).

    :param key_converter:
        Converter used for keys. Defaults to a no-op.
    """

    #: Unit generator registry.
    registry: dict[Hashable, UnitGenerator | pint.Unit | str] = field(
        default_factory=dict
    )

    #: String-to-units interpretation switch.
    interpret_str: bool = False

    #: Unit registry used for string-to-units interpretation.
    ureg: pint.UnitRegistry | None = None

    #: Converter used for keys.
    key_converter: Callable[[Hashable], Hashable] = identity
    ...
    def _generator(self, key: Hashable) -> UnitGenerator:
        """
        Return the generator registered under an already-converted key.

        Registry values are normalized to :class:`.UnitGenerator` by
        :meth:`_convert_value`; the declared type of ``registry`` describes what
        the constructor accepts, not what is stored.
        """
        return cast(UnitGenerator, self.registry[key])
    ...
    def _convert_key(self, key):
        ...
        :returns:
            The converted key.
        """
        converted = self.key_converter(key)
        self.registry[converted] = self.registry.pop(key)
        return converted
    ...
    def _convert_value(self, key):
        ...
        :param key:
            Key to the value to which conversion is to be applied. It must
            already have been through ``key_converter``.
        """
        ...
        if isinstance(value, pint.Unit):
            ...
            raise TypeError(
                "Items must be either str, pint.Unit or UnitGenerator; got: "
                f"{key}: {type(value)}"
            )

    def register(self, key: Hashable, value: UnitGenerator | pint.Unit | str) -> None:
        """
        ...
        * ``value`` is converted to a :class:`UnitGenerator`.
        ...
        """
        self.registry[key] = value
        # Reminder: The following operations mutate the registry
    ...
    def get_all(self) -> dict[Hashable, pint.Unit]:
        ...
        """
        return {key: self.get(key) for key in self.registry}
    ...
    @contextmanager
    def override(self, *args, **kwargs) -> Generator[None]:
        """
        ...
        * key-value pairs may also be specified as keyword arguments.
        ...
        .. note::
            When using the keyword argument specification, passed values
        ...
        with ExitStack() as stack:
            for arg in args:
                ...
                for key, value in arg.items():
                    stack.enter_context(
                        self._generator(self.key_converter(key)).override(value)
                    )
    ...
    def __setitem__(
        self, key: Hashable, value: UnitGenerator | pint.Unit | str
    ) -> None:
        """
